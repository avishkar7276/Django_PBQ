...
from .models import Blog, Blog_Category, Photo, Photo_Category,Quotes_Category,Quotes
...
def home(request):
    ...
    photos = Photo.objects.all().order_by("-id")[:3]
   # print("Photo ========>",photos)
    context = {'Pic': photos, 'posts': posts,'number':3}
...
def dashboard(request):
    if request.user.is_authenticated:
        ...
        gps = user.groups.all()
        return render(request, 'blog/dashboard.html', {'photos': photos, 'posts': posts, 'full_name': full_name, 'groups': gps,'Quotes':quotes})
    else:
...
def addQuotes(request):
    categories = Quotes_Category.objects.all()
    user1 = request.user
    if request.method == 'POST':
        data = request.POST
        if data['category'] != 'none':
            category1 = Quotes_Category.objects.get(id=data['category'])
        elif data['category_new'] != '':
            category1, created =Quotes_Category.objects.get_or_create(
                name=data['category_new'])
        else:
            category = None
       # print(data)
       # print('catgory ===============>',category1)
        bolog = Quotes.objects.create(Category=category1,Content=data['Content'],user=user1)

        return redirect('dashboard')

    context = {'categories': categories}
    return render(request, 'quotes/addQuotes.html',context)
...
def view_blogs(request):
    ...
    context = {'posts': posts}
    return render(request, 'blog/blog_site.html', context)
...
def view_photo(request):
    ...
    context = {'Pic': photos}
    return render(request, 'photos/photo_site.html', context)